...
function within(date, startStr, endStr){
  if(!date) return false
  if(!startStr && !endStr) return true
  const d = new Date(date); d.setHours(0,0,0,0)
  if(startStr){ const s = new Date(startStr); s.setHours(0,0,0,0); if(d < s) return False }
  if(endStr){ const e = new Date(endStr); e.setHours(23,59,59,999); if(d > e) return false }
  return true
}
...
export default function Solicitacoes(){
  ...
  return (
    <div className="grid" style={{gap:20}}>
      <div>
        <h1 className="section-title">Solicitações de Transporte</h1>
        <div className="section-sub">Visualize e pesquise todas as solicitações de transporte</div>
      </div>
      ...
      <div className="card">
        <div className="card-b" style={{overflowX:'auto'}}>
          <table className="table">
            ...
            <tbody>
              {filtradas.length===0 ? (
                <tr><td colSpan="9" style={{textAlign:'center', padding:'16px 0'}} className="muted">Nenhuma solicitação encontrada</td></tr>
              ): filtradas.map(s=>{
